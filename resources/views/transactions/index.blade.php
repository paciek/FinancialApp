...
<div class="container py-4">
            ...
            <div class="card border-0 shadow-sm bg-success-subtle h-100">
                <div class="card-body">
                    <div class="d-flex align-items-center gap-2 mb-2">
                        ...
                        <span class="fw-semibold">Suma przychodów</span>
                    </div>
                    <div class="fs-5 fw-bold text-success">
                        {{ number_format($totalIncome, 2) }} {{ $currency }}
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card border-0 shadow-sm bg-danger-subtle h-100">
                <div class="card-body">
                    <div class="d-flex align-items-center gap-2 mb-2">
                        ...
                        <span class="fw-semibold">Suma wydatków</span>
                    </div>
                    <div class="fs-5 fw-bold text-danger">
                        {{ number_format($totalExpense, 2) }} {{ $currency }}
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card border-0 shadow-sm h-100 {{ $balance >= 0 ? 'bg-primary-subtle' : 'bg-warning-subtle' }}">
                <div class="card-body">
                    <div class="d-flex align-items-center gap-2 mb-2">
    ...
    @if($transactions->total() === 0)
        <div class="alert alert-secondary">Brak danych do podsumowania</div>
    @endif

    @if(request('q'))
        <div class="alert alert-info">
            Wyniki dla: <strong>{{ request('q') }}</strong> ({{ $transactions->total() }} znalezionych)
        </div>
    @endif
    ...
    @php
        ...
        $iconForDate = $currentSort !== 'transaction_date' ? 'fa-sort' : ($currentDirection === 'asc' ? 'fa-sort-up' : 'fa-sort-down');
        $iconForAmount = $currentSort !== 'amount' ? 'fa-sort' : ($currentDirection === 'asc' ? 'fa-sort-up' : 'fa-sort-down');
    @endphp
        ...
        <table class="table mb-0 align-middle">
            ...
            @forelse($transactions as $transaction)
                <tr>
                    ...
                    <td>{{ number_format((float) $transaction->amount, 2) }} {{ $currency }}</td>
                    <td>
                    ...
                    <td>
                        <div class="d-flex gap-2">
                            ...
                            <form action="{{ route('transactions.destroy', $transaction) }}" method="POST" onsubmit="return confirm('Czy na pewno chcesz usunąć transakcję?');">
                                @csrf
                                @method('DELETE')
